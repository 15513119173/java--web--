...
<form name="studentForm" action="student?action=studentDel" method="post">
&nbsp;
<table width="96%"  border="1" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF" bordercolordark="#FFFFFF" bordercolorlight="#67A8DB">
  ...
  <%List list=((List<StudentForm>)request.getAttribute("studentQuery"));
    for(int i=0;i<list.size();i++){
         StudentForm bean=(StudentForm)list.get(i);
   %>
  <tr>
 	<td align="center"><%=bean.getID() %></td> 
    <td style="padding:5px;"><%=bean.getName() %></td>
	<td align="center"><%=bean.getSex() %></td>
	<td align="center"><%=bean.getJoinTime() %></td>
    <td align="center"><%=bean.getQuestion() %></td>	
    <td align="center"><%=bean.getCardNo() %></td>
    <td align="center"><input name="delIdArray" type="checkbox" class="noborder" value="<%=bean.getID()%>"/>
    <input name="ID" type="hidden" class="noborder" value="<%=bean.getID()%>"/>
    </td>
  </tr>
  <%} %>
  </logic:iterate> 
